...
# 🍔 Product API — Multer + Firm Relation — Complete Guide

## 📌 **addProduct** — Add New Product With Image Upload

This API lets you add a **new product**, upload an **image**, and link the product to a **firm**.
...
* `upload.single('image')` → Uploads one image file
* `:firmId` → Firm ID to link product
...
```
POST /add-product/65ab12cd
Form-Data:
- productName: Pizza
- price: 199
- categorey: Italian
- bestseller: true
- description: Hot cheese pizza
- image: <file>
```

---

## ✔️ addProduct — Explanation

```js
const image = req.file ? req.file.filename : undefined;
```

✔ Saves uploaded image filename

```js
const firm = await Firm.findById(firmId);
```

✔ Checks if firm exists
...
✔ Pushes product into firm's product array
...
| Feature             | Purpose                       |
| ------------------- | ----------------------------- |
| Image upload        | Store product images (Multer) |
| Link product → firm | One-to-many relation          |
| Auto-save to firm   | Avoid extra queries           |
| Filename handling   | Unique images using timestamp |
...
* For real projects → use **Cloudinary / S3**
* Always check if firm exists before saving

---

# 📌 **getProductByFirm** — Fetch Products of a Firm

This API returns **all products** that belong to a specific firm.

---

## ⭐ Definition

Fetch products by using:

```js
Product.find({ firm: firmId })
```
...
```json
{
  "restaurantName": "Ramesh Foods",
  "products": [
    { "productName": "Pizza", "price": 199 },
    { "productName": "Burger", "price": 149 }
  ]
}
```
...
* Display all products of a particular restaurant/firm
* Fetch details dynamically using `firmId`

---

# 📌 **deletedProductById** — Delete Product
...
* Remove unwanted or discontinued products
...
Multer is a **Node.js middleware** used to handle **image/file uploads** using `multipart/form-data`.
...
| Purpose             | Description             |
| ------------------- | ----------------------- |
| Upload Images       | Product photos, banners |
| Unique Filenames    | No overwriting          |
| Local Storage       | Quick development setup |
| Can integrate cloud | Cloudinary, AWS S3      |

---

## 🔥 Tricks

* Always store uploads outside project root in production
* Validate file type using `fileFilter`
* Use Cloudinary for real deployment

---

# 📝 Summary

* **addProduct** lets you upload images + create a product + link to firm.
* **getProductByFirm** fetches all products of a specific firm.
* **deletedProductById** removes a product from DB.
* **Multer** handles image uploads with disk storage.
* Use `populate()` or direct queries to manage firm-product relations.

---

If you want, I can add:
🔹 Cloudinary upload version
🔹 Relation diagram (Firm → Products)
